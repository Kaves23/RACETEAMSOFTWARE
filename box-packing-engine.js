...
(function() {
  ...
  async function selectBox(boxId) {
    ...
    try {
      ...
      const rawContents = contentsResp?.boxContents || contentsResp?.contents || [];
      if (contentsResp && contentsResp.success && Array.isArray(rawContents)) {
        boxContents = rawContents.map(c => ({
          id: String(c.id),
          boxId: c.box_id,
          itemId: c.item_id,
